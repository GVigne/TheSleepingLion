...
class MultilineCommand(AbstractItem):
    """
    A class which is basically just a wrapper around a ColumnItem. Allows the user to show elements stacked one
    ...
    def __init__(self, arguments: list[str],
        ...
        self.column = GloomhavenColumnItem(self.parsed_arguments, self.path_to_gml)
...
class EnhancementDotCommand(AbstractItem):
    """
...
class InsideCommand(AbstractItem):
    """
...
class ExpCommand(AbstractItem):
    ...
    then the word 'exp' will never be passed to the parser, as the lexer will replace it with something else. It's
    ...
    def __init__(self, arguments: list[str|AbstractItem],
        ...
        context = GloomhavenLineContext(font = title_font,
                                font_size = font_size_factor*gml_context.font_size,
                                text_color={"red": 0, "green": 0, "blue": 0})
        self.exp_value = TextItem(arguments, context, path_to_gml=path_to_gml)
...
class OneChargeItem(AbstractItem):
    """
    ...
    def __init__(self, arguments: list[str|AbstractItem],
        ...
        self.one_charge = SVGImage(get_image(self.path_to_gml, "one_charge.svg", Haven.GLOOMHAVEN),
                                        ...
                                        new_color=gml_context.class_color) # Replace the black arrow with an arrow of same color as the class
            ...
            if len(charge_effect_lst) > 0:
                ...
                if isinstance(charge_effect.items[0], ExpCommand):
                    ...
                    charge_effect.items[0].new_image(SVGImage(get_image(self.path_to_gml, "experience.svg", Haven.GLOOMHAVEN),
                                                                ...
                                                                new_color=gml_context.class_color))
    ...
    def get_circle_center(self):
        """
        Return the distance between the left of the "one_charge" image and its circle's center.
        """
...
class ChargesCommand(AbstractItem):
    """
    ...
    def __init__(self, arguments: list[str|AbstractItem],
        ...
        self.blank = TextItem(["  "], GloomhavenLineContext(font_size = base_font_size))
        ...
        for (i,arg) in enumerate(self.arguments):
            ...
            one_charge = OneChargeItem(charge_effect, gml_context, self.path_to_gml)
    ...
    def draw(self, cr: cairo.Context):
        ...
        first_line = GloomhavenLineItem(list_join([self.infinity] + first_args, self.blank), None)
        cr.save()
        first_line.draw(cr)
        cr.restore()

        if len(snd_args) > 0:
            second_line = GloomhavenLineItem(list_join(snd_args, self.blank), None)
            cr.save()
...
class SummonCommand(AbstractItem):
    ...
    def __init__(self, arguments: list[str],
        ...
        self.hp_line = GloomhavenLineItem(hp_image + [self.parsed_arguments[1]])
        self.move_line = GloomhavenLineItem(move_image + [self.parsed_arguments[2]])
        self.attack_line = GloomhavenLineItem(attack_image + [self.parsed_arguments[3]])
        self.range_line = GloomhavenLineItem(range_image + [self.parsed_arguments[4]])
...
class DividerLine(AbstractItem):
    """
    ...
    def __init__(self,
        ...
        self.dotted_line = TextItem([divider_message], GloomhavenLineContext(font_size=small_font_size),self.path_to_gml)
...
# create_command -> AbstractItem(args) -> create_command(args[0]) -> ...
