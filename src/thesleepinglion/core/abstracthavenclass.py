...
class AbstractHavenClass:
    ...
    def __init__(self, path_to_gml : Path, card_type: type):
        ...
        self.card_type = card_type # Think of this class as a class template: card_type is a template parameter
                                    # which can be used to create an instance of a child class of AbstractCard
        ...
        # Try to retrieve the name, path ... from the class section. If it isn't there (the user didn't specify it)
    ...
    def create_card_layout(self, cr: cairo.Context):
        """
        Draw the general layout of a card onto the given cairo context, such as the background, class icon, arrow
        above the initiative....
        This method must be overloaded by child classes.
        """
        pass

    def save_header(self, format_id: str):
        """
        Internal function called when serialising a class, before anything else happens. Write a header in the GML
        file in order to know if Frosthaven or Gloomhaven syntax is used.
        This method must be overloaded by child classes, so that save_header can be called without the "format" argument.
        """
    ...
    def check_class_errors(self):
        ...
        This method may be overloaded by child classes to add warnings or errors specific to Gloomhaven or Frosthaven formatting.
        """
        class_warnings = []
    ...
    def save(self):
        ...
        """
        open(self.path_to_gml, 'w').close() # Clear the GML file
        self.save_header()
    ...
    def parse_gml(self):
        """
        Parse the top and bottom actions for every card. Return errors as two dictionnaries with keys being the name
    ...
    def draw_card(self, card : AbstractCard|None ,cr: cairo.Context):
        """
    ...
    def delete_card(self, card: AbstractCard):
        """
    ...
    def set_path_to_gml(self, new_path: Path):
        """
        Set the path to GML file to a new path.
        """
        self.path_to_gml = new_path
        for card in self.cards:
            card.path_to_gml = new_path

    def valid_rename(self, card_name: str, existing_card: AbstractCard|None):
        """
        ...
        If another name should be picked, it will be something like "Card 42".
        Note: some card names are also blacklisted (already used for fields in GML). Furthermore, a card may not have
        an empty name.
        """
        matching_card = None
        for card in self.cards:
        ...
        blacklisted = ["class", "aliases", "format", "special rules"]
    ...
    def cards_from_gml(self):
        ...
        for i, (card_name, instructions) in enumerate(self.python_gml.items()):
            if card_name !="class" and card_name !="aliases":
                ...
                level = str(instructions.get("level",""))
                card_id = str(instructions.get("ID",""))
