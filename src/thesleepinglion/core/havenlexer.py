...
class HavenLexer:
    ...
    representing the top/bottom action of a card. Namely, the lexer should be able to:
        ...
        - split a line into lexemes: text, commands (starting with a backslash) or macros (starting with an @)
    When separating the lines, the lexer should also substitute all aliases.
    """
    def __init__(self, base_aliases: list[str]):
        """
        Pass as an argument the list of aliases from the "standard library". It's the only difference between
        a Gloomhaven parser and a Frosthaven parser.
        """
        self.base_aliases = base_aliases
    ...
    def include_aliases(self, text : str, additional_aliases: str = ""):
        ...
        all_aliases = alias_text_to_alias_list(additional_aliases) + self.base_aliases
